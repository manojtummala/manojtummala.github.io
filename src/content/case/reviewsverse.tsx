  ...
  return (
    <article className="space-y-6">
      <header className="space-y-2">
        <h1 className="text-2xl font-bold text-slate-800">ReviewsVerse — Topic-tagged reviews platform</h1>
        <p className="text-sm text-slate-600">Role: Full-Stack • Period: May–Sep 2024</p>
        <div className="flex flex-wrap gap-2 text-xs">
          {["Next.js","Node.js","MongoDB","Kafka","S3","Docker"].map(s=>(
            <span key={s} className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-700 border border-slate-200">{s}</span>
          ))}
        </div>
      </header>

      <section>
        <h2 className="text-lg font-semibold text-slate-800">TL;DR</h2>
      ...
      <section>
        <h2 className="text-lg font-semibold text-slate-800">What I&#39;d improve</h2>
        <ul className="mt-2 list-disc pl-5 space-y-1 text-sm text-slate-700">
